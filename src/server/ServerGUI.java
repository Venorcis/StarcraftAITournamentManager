	...
	private void handleNoGamesFile()
	{
		...
		if (!new File(ServerSettings.Instance().GamesListFile).exists())
		{
			...
			if (generate == JOptionPane.YES_OPTION)
			{
				...
				GameListGenerator.GenerateGames(Integer.parseInt("" + spinner.getValue()), ServerSettings.Instance().MapVector, ServerSettings.Instance().MapSelection, ServerSettings.Instance().BotVector, ServerSettings.Instance().TournamentType);
